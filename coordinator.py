...
def mutual_exclusion(received_message, addr):
    if received_message == REQUEST_MESSAGE:
        if len(Q) == 0:
            new_message = create_message("999", GRANTED_MESSAGE)
            sock.sendto(new_message.encode(), (addr))
...
while True:
    ...
    mutual_exclusion_thread = threading.Thread(target=mutual_exclusion(received_message, addr))
